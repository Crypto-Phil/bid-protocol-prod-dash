...
async function build() {
  ...
  const agents = await query(client,
  ...
  const ledger = await query(client,
  ...
  const agentPnl = await query(client, `
    SELECT agent_address, agent_name,
      ...
      sum(bankroll_wei) as total_bankroll,
      avg(bankroll_wei) as avg_bankroll
  ...
  for (const [name, info] of Object.entries(ACCOUNTS)) {
    ...
    const agentsWithMeta = acctPnl.map(p => {
      ...
      return {
        ...
        battle_limit_usdc: agentMeta?.battle_limit_usdc || null,
        avg_bankroll: Math.round(parseFloat(p.avg_bankroll)),
      };
    });
    ...
    result[name] = {
      ...
      agents: agentsWithMeta,
    };
  }

  result._built_at = new Date().toISOString();

  fs.writeFileSync(path.join(__dirname, 'public', 'data.json'), JSON.stringify(result, null, 2));
  console.log('Built public/data.json at', result._built_at);
